...
cheating_detection_model = load_model('BehaviorClassification_90%.h5')
...
while cap.isOpened():
    ...
    if len(time_steps_buffer) == 30:
        ...
        predicted_class = np.argmax(cheating_prediction, axis=1)[0]  # Get the predicted class index
        cheating_type = ""  # Initialize an empty string for cheating type

        if predicted_class == 1:
            cheating_text = "Cheating Type 1"
            cheating_type = "Type 1"
        elif predicted_class == 2:
            cheating_text = "Cheating Type 2"
            cheating_type = "Type 2"
        else:
            cheating_text = "Not Cheating"
        # Display the cheating detection result
        cv2.putText(image, cheating_text, (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        if cheating_type:
            cv2.putText(image, cheating_type, (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
